...
(m:={(x,y) for s in stdin.readlines() for z in [map(int,findall('\d+',s))] for p in [[*zip(z,z)]] for (a,b),(c,d) in zip(p,p[1:]) for x in range(min(a,c),max(a,c)+1) for y in range(min(b,d),max(b,d)+1)})
(M:=len(m))
(h:=max(v[1] for v in m))
